...
LAC2 <- NATIONAL_DB |>
    filter(C3 == "LATIN-AMERICA") |>
    mutate(
        alpha = ifelse(C2 %in% c("Mexico", "Costa Rica", "Brazil", "Colombia", "Uruguay", "Dominican Republic") & S2 == "IPUMS", "1", "0"),
...
C <- ggplot(LAC2, aes(x = T1, y = prop_ch, group = interaction(C2, S2), label = C2)) +
    geom_line(aes(alpha = alpha), linewidth = 0.5, show.legend = FALSE, color = "#118F5E") +
...
ggsave("III_distribucion_tipo.png", plot = E, scale = 1, dpi = 500, 
    height = 10, width = 14)
...
ggsave("IV_correlacion.png", plot = E, scale = 1, dpi = 500, height = 8, width = 10)
